/* ═══════════════════════════════════════════════════════
   NEMESIS — SCRIPT v4
   Kindle-only · GSAP ScrollTrigger · Book Open Animation
   ═══════════════════════════════════════════════════════ */
...
(() => {
  ...
  function setupCursor() {
    ...
    document.querySelectorAll(
      'a, button, .ch-card, .o-card, .c-item, .crew-cell, .id-row, .kfs-item, .mini-cover, .kf-cover img'
    ).forEach(el => {
  ...
  /* ════════════════════════════════════════
     ...
     Pinned · 300vh scroll travel
     Phase 1 (0–25%)  : book scales up, cue fades
     Phase 2 (25–75%) : cover rotates open -180°
     Phase 3 (75–100%): inside spread appears
     After leave      : site-main slides up
  ════════════════════════════════════════ */
  function setupBookAnimation() {
    ...
    entryTl
      .to(bookWrap,    { scale: 1, opacity: 1, duration: 1.5, ease: 'power3.out' })
      .to(ambientWord, { opacity: 1, scale: 1, duration: 2.0, ease: 'power2.out' }, '-=1.0')
      .to(scrollCue,   { opacity: 1, duration: 0.8, ease: 'power2.out' }, '-=0.5');

    /* Mouse parallax tilt (only while at top) */
    let tiltActive = true;
    ...
    stage.addEventListener('mouseleave', () => {
      if (!tiltActive) return;
      gsap.to(bookWrap, { rotateY: -18, rotateX: 4, duration: 0.85, ease: 'power3.out', overwrite: 'auto' });
    }, { passive: true });
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        scrub: 1.4,
        anticipatePin: 1,
        onUpdate: self => { tiltActive = self.progress < 0.02; },
        onLeave: () => {
          gsap.to(siteMain, { y: 0, opacity: 1, duration: 1.1, ease: 'power3.out' });
        },
        onEnterBack: () => {
          gsap.to(siteMain, { y: 80, opacity: 0, duration: 0.5, ease: 'power2.in' });
        }
      }
    });
    ...
    tl
      ...
      .to(bookWrap,   { scale: 1.25, rotateY: -6, rotateX: 1, duration: 0.22, ease: 'power2.inOut' }, '<')
      .to(ambientWord,{ opacity: 0, scale: 0.9, duration: 0.14 }, '<0.04')
      .to(stageBeam,  { opacity: 1, duration: 0.08 }, '<0.05')

      /* Phase 2 — cover opens */
      .to(bookCover,  { rotateY: -180, duration: 0.52, ease: 'power2.inOut' })
      .to(bookWrap,   { x: 90, duration: 0.52, ease: 'power2.inOut' }, '<')

      /* Phase 3 — inside spread */
  ...
  function setupSectionReveals() {
    ...
    gsap.utils.toArray('.s-eyebrow, .s-title, .s-rule, .intro-para, .contact-intro, .ig-lead, .ig-body, .kf-title, .kf-author').forEach(el => {
      gsap.from(el, {
    ...
    /* Kindle feature cover */
    const kfCover = document.querySelector('.kf-cover');
    if (kfCover) {
      gsap.from(kfCover, {
        x: -40, opacity: 0, duration: 1.1, ease: 'power3.out',
        scrollTrigger: { trigger: kfCover, start: 'top 85%', toggleActions: 'play none none none' }
      });
    }

    /* Kindle spec items */
    const kfsItems = document.querySelectorAll('.kfs-item');
    if (kfsItems.length) {
      gsap.from(kfsItems, {
        y: 20, opacity: 0, duration: 0.65, ease: 'power3.out', stagger: 0.05,
        scrollTrigger: { trigger: '.kf-specs', start: 'top 85%', toggleActions: 'play none none none' }
      });
    }

    /* Kindle feature checks */
    const kffItems = document.querySelectorAll('.kff-item');
    if (kffItems.length) {
      gsap.from(kffItems, {
        x: -16, opacity: 0, duration: 0.5, ease: 'power2.out', stagger: 0.06,
        scrollTrigger: { trigger: '.kf-features', start: 'top 88%', toggleActions: 'play none none none' }
      });
    }
    ...
    [
      ...
      Object.values(bySection).forEach(group => {
        ...
        gsap.from(group, {
          y: 24, opacity: 0, duration: 0.7, ease: 'power3.out', stagger,
  ...
  /* ════════════════════════════════════════
     CHARACTERS HORIZONTAL DRAG TRACK
  ════════════════════════════════════════ */
  function setupCharactersTrack() {
    ...
    vp.addEventListener('mousedown', e => {
      down = true; startX = e.clientX; startLeft = vp.scrollLeft;
